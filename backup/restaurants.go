...
func GetRestaurantsTable(ctx *context.Context) table.Table {
	...
	info.AddField("Opening_time", "opening_time", db.Time).
		FieldDisplay(func(value types.FieldModel) interface{} {
			// If it's already a string, we ensure it's truncated to 8 characters
	...
	info.AddField("Closing_time", "closing_time", db.Time).
		FieldDisplay(func(model types.FieldModel) interface{} {
			if t, ok := model.Row["closing_time"].(time.Time); ok {
	...
	info.AddField("Estimated_delivery_time", "estimated_delivery_time", db.Varchar)
